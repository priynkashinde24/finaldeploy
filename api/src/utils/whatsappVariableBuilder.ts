...
/**
 * WhatsApp Template Variable Builder
 *
 * PURPOSE:
...
export async function buildSupplierVariables(
  ...
  // Add label link if available (from shipping label system)
  if (order.metadata?.shippingLabelUrl) {
    variables.labelLink = order.metadata.shippingLabelUrl;
  } else {
    // Fallback: build label link from order number
    variables.labelLink = `${process.env.FRONTEND_URL}/admin/shipping/labels/${order.orderNumber}`;
  }
...
function calculateResellerMargin(order: IOrder): number {
  ...
  for (const item of order.items) {
    ...
    margin += itemMargin * item.quantity;
  }
