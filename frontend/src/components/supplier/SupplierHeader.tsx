...
export const SupplierHeader: React.FC = () => {
  ...
  const handleLogout = async () => {
    try {
      ...
      console.error('[SUPPLIER HEADER] Logout error:', error);
  ...
  return (
    <header className="fixed top-0 right-0 left-0 lg:left-64 h-16 bg-surface border-b border-border z-30 flex items-center justify-between px-4 lg:px-6">
      ...
      <div className="flex-1">
        <h2 className="text-lg font-semibold text-white">Supplier Panel</h2>
      </div>

      {/* Right side - Logged-in supplier email and logout */}
